-- Allow editing an existing sales bill (lines, pricing, VAT, extra charges, header).
-- RETURNS TABLE(bill_id, bill_no) defines PL/pgSQL vars with those names — qualify
-- table columns (pay.bill_id, si.bill_id) or Postgres errors: "bill_id is ambiguous".
-- Does not change `paid`; use customer payments / returns for that.
-- Fails if the new total would be less than amount already paid on the bill.
